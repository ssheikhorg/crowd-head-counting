...
def process_and_resize(files, output_dir, split):
    ...
    for img_path, mat_file, dataset in tqdm(files, desc=f'Processing {split}'):
        ...
        if dataset in ['shanghai', 'ucf']:
            ...
            points = mat['image_info'][0][0][0][0][0] if dataset == 'shanghai' else mat['annPoints']

            with open(label_path, 'w') as f:
                for point in points:
                    x, y = point[0] * TARGET_SIZE / img.shape[1], point[1] * TARGET_SIZE / img.shape[0]
                    f.write(f"0 {x / TARGET_SIZE:.6f} {y / TARGET_SIZE:.6f} 0.01 0.01\n")
